 ...
 * OPTIMIZADO: 1 sola query con OR en lugar de 2 queries secuenciales.
 * Prioriza misma categoría (aparecen primero en el resultado), complementa con otras.
 */
export const getRelatedArticles = cache(async (currentId, category, limit = 6) => {
  ...
  // 1 query: OR entre misma categoría Y otras categorías, ordenadas por relevancia
  // La misma categoría sube primero gracias al order por publishedAt
  const { data } = await supabase
    .from('articles')
    .select(fields)
    .neq('id', currentId)
    .or(`category.eq.${category},category.neq.${category}`)
    .order('category', { ascending: false }) // agrupa la misma categoría primero
    .order('publishedAt', { ascending: false })
    .limit(limit * 3); // Traer más y filtrar en memoria — 1 round-trip vs 2

  if (!data?.length) return [];

  // Priorizar misma categoría, completar con otras
  const same    = data.filter(a => a.category === category).slice(0, limit);
  const others  = data.filter(a => a.category !== category).slice(0, limit - same.length);
...
export const getDailyTopArticles = cache(async (limit = 12, minRequired = 6) => {
  ...
  // Compatibilidad: mantener nombre mostViewedToday = null para no cambiar lógica siguiente
  const mostViewedToday = null;
