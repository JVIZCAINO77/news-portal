 ...
 *   2. Intenta subir via Cloudinary — usa autenticación HMAC (apiKey/apiSecret) o preset.
...
export async function internalizeImage(externalUrl) {
  ...
  // Si no hay configuración mínima, no podemos subir
  ...
  try {
    ...
    if (uploadPreset) {
      ...
      // Autenticación basada en firma si no hay preset
