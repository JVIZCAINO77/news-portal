...
export function optimizeImageUrl(url, width = 1200) {
  ...
  // Cloudinary: transform rápido — solo f_auto,q_auto sin crop inteligente
  if (url.includes('cloudinary.com')) {
    try {
      if (url.includes('/upload/')) {
        // Si ya tiene f_auto y w_ no tocar (evita dobles transforms)
        if (url.includes('f_auto') && url.includes('w_')) return url;
        // Transform mínimo: formato auto + calidad auto + ancho. SIN c_fill ni g_auto (son lentos)
  ...
  // Para Unsplash no necesitamos proxy — ya tiene CDN propio con parámetros nativos
  if (url.includes('unsplash.com')) {
    return `${url.split('?')[0]}?w=${width}&q=80&auto=format&fit=crop`;
  }

  // Resto de imágenes externas: proxy weserv.nl para evitar hotlinking
  return `https://images.weserv.nl/?url=${encodeURIComponent(url)}&w=${width}&fit=cover&q=80`;
}
