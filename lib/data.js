...
export function optimizeImageUrl(url, width = 1200) {
  ...
  // Cloudinary — soporte básico de ancho
  if (url.includes('cloudinary.com')) {
    try {
      if (url.includes('/upload/')) {
        ...
        // Si el ancho es pequeño (ej. blur), forzamos una miniatura rápida
        if (width <= 100) {
          return url.replace('/upload/', `/upload/c_scale,w_${width},q_auto:low/`);
        }
        
        // Limpieza de transforms previos para servir la imagen limpia
  ...
  if (url.includes('unsplash.com')) {
    return `${url.split('?')[0]}?w=${width}&q=80&auto=format&fit=max`;
  }
