...
export function optimizeImageUrl(url, width = 1200) {
  ...
  if (!url.includes('cloudinary.com')) {
    // Si es una imagen externa (no cloudinary), usamos un proxy para evitar bloqueos por hotlinking
    // y para redimensionar (ahorro de ancho de banda).
    return `https://images.weserv.nl/?url=${encodeURIComponent(url)}&w=${width}&fit=cover&q=85`;
  }
