...
export async function postToSocialMedia(article) {
  ...
  // Para Instagram: SIEMPRE usar la imagen directa de Cloudinary.
  // Meta no puede acceder a rutas dinámicas de Next.js (/api/og) — verificado por debug.
  const igImageUrl = article.image?.split('?')[0] || null;
  if (igImageUrl) {
    console.log('[Social] 📸 Instagram imagen (Cloudinary):', igImageUrl.slice(0, 70));
  }
