...
export async function postToSocialMedia(article) {
  ...
  // Para Instagram: necesitamos una URL estática accesible por Meta.
  // Intentamos calentar la OG primero; si falla usamos la imagen directa de Cloudinary.
  let igImageUrl = article.image?.split('?')[0] || null; // Fallback: imagen real Cloudinary
  try {
    // Pre-calentar la OG para que Vercel la cachée antes de que Meta la intente bajar
    const warmRes = await fetch(thumbnailUrl, { method: 'GET' });
    if (warmRes.ok) {
      igImageUrl = thumbnailUrl; // Usar la miniatura brandizada
      await new Promise(r => setTimeout(r, 1500)); // Dar tiempo al CDN
      console.log('[Social] 🖼️ OG thumbnail pre-cacheado para Instagram.');
    }
  } catch (_) {
    console.warn('[Social] ⚠️ OG warm-up falló — usando imagen directa de Cloudinary para Instagram.');
  }
        ...
        for (let attempt = 0; attempt < 6 && status !== 'FINISHED'; attempt++) {
          ...
          try {
            const statusRes = await fetch(
              `https://graph.facebook.com/v19.0/${containerData.id}?fields=status_code&access_token=${process.env.FACEBOOK_PAGE_TOKEN}`
            );
