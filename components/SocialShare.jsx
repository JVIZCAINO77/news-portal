...
export default function SocialShare({ url, title }) {
  ...
  const channels = [
    ...
    },
    {
      ...
      extraClick: (url) => { try { navigator.clipboard.writeText(url); } catch (_) {} }
    },
  ];
         ...
         <div className="flex flex-col gap-6">
            {channels.map((ch) => (
              <a
                ...
                onClick={() => {
                  trackShare(ch.name, title);
                  if (ch.extraClick) ch.extraClick(shareUrl);
                }}
         ...
         <div className="flex justify-between items-center gap-8">
            {channels.map((ch) => (
              <a
                ...
                className={`transition-all duration-300 ${ch.name === 'WhatsApp' ? 'text-emerald-500 scale-125' : ch.name === 'Instagram' ? 'text-pink-400' : 'text-white/60'}`}
                aria-label={ch.name}
              >
                {ch.icon}
              </a>
